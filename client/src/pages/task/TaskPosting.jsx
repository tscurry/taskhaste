...
const TaskPostingForm = () => {
  ...
  return (
    <div className="flex flex-col justify-center items-center bg-white h-screen w-full overflow-x-hidden">
      <div className="w-[95%] mx-5 mb-10">
        <h1 className="font-bold text-xl lg:text-2xl mt-20 text-center">Let's Get Started With Your Task</h1>
        <p className="mt-6 md:text-sm text-xs text-center">Fill in the details below and find a capable helper for your task.</p>
        <p className="md:text-sm text-xs text-center">Quick, simple and secure. Let's get things done!</p>
        <form action="/task-posting" method="post" className="mt-[120px]" onSubmit={handleFormSubmit}>
          <div className="flex flex-col sm:w-[500px] md:w-[600px] lg:w-[700px] mx-auto">
            ...
            {completedSteps.includes(3) && (
              <>
                ...
                <div className="flex w-full mt-10 gap-5 sm:gap-8 lg:gap-12 max-xs:flex-wrap">
                  <div className="flex flex-col w-1/2 max-xsm:w-full">
                ...
                <input
                  type="submit"
                  value="Post Task"
                  className="bg-primary text-white p-2 text-sm rounded disabled:bg-[#ccc] disabled: my-10 hover:scale-[102%] disabled:transition-none disabled:scale-100 transition-transform duration-500 cursor-pointer"
                  disabled={!validForm()}
                />
              </>
            )}
